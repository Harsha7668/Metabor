...
async def ytdlleech_handler(client: Client, msg: Message):
    ...
    ydl_opts = {
        ...
        'force_generic_extractor': True,
        'noplaylist': True,
        'progress_hooks': []  # Initialize empty list for progress hooks
    }
...
async def callback_query_handler(client: Client, query):
    ...
    ydl_opts = {
        ...
        'quiet': True,
        'noplaylist': True,
        'progress_hooks': [await progress_hook(sts)],  # Await the progress hook correctly
        'merge_output_format': 'mp4'  # Ensure the output is in mp4 format
    }
    download_path = os.path.join(DOWNLOAD_LOCATION, f"{video_title}.mp4")
    file_thumb = None
...
@Client.on_message(filters.private & filters.command("zee5leech"))
async def zee5leech_handler(client: Client, msg: Message):
    if len(msg.command) < 2:
        return await msg.reply_text("Please provide a Zee5 link.")
    ...
    await msg.reply_text("Please enter your Zee5 phone number:")
    user_credentials[user_id]['step'] = 'phone_number'
...
async def credentials_handler(client: Client, msg: Message):
    ...
    if step == 'phone_number':
        user_credentials[user_id]['phone_number'] = msg.text
        await msg.reply_text("Please enter the OTP sent to your phone:")
        user_credentials[user_id]['step'] = 'otp'
    elif step == 'otp':
        user_credentials[user_id]['otp'] = msg.text
...
async def process_zee5_download(client: Client, msg: Message, credentials: dict):
    ...
    # Use phone number and OTP to authenticate and obtain cookies
    cookies = authenticate_zee5(phone_number, otp)
...
async def callback_query_handler(client: Client, query):
    ...
    ydl_opts = {
        ...
        'outtmpl': os.path.join(DOWNLOAD_LOCATION2, f"{video_title}.mkv"),
        'quiet': True,
        'noplaylist': True,
    ...
    try:
        ...
        if thumbnail_url:
            thumbnail_path = f"{DOWNLOAD_LOCATION2}/thumbnail_{query.from_user.id}.jpg"
            ydl_opts_thumbnail = {'outtmpl': thumbnail_path}
            with YoutubeDL(ydl_opts_thumbnail) as ydl_thumb:
                ydl_thumb.download([thumbnail_url])
            file_thumb = thumbnail_path
...
async def zee5leech_handler(client: Client, msg: Message):
    ...
    try:
        with YoutubeDL(ydl_opts) as ydl:
            ...
            if not formats:
                raise ValueError("No downloadable formats found. The content might be DRM protected or requires a subscription.")
        ...
        error_message = str(e)
        if 'HTTP Error 475' in error_message:
            error_message = "The content might be DRM protected or requires a subscription."
        await msg.reply_text(f"Error: {error_message}")

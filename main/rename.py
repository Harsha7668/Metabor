...
async def change_index_audio(bot, msg):
    global CHANGE_INDEX_ENABLED, GDRIVE_FOLDER_ID
    DOWNLOAD_LOCATION = "downloads"
    FILE_SIZE_LIMIT = 2 * 1024 * 1024 * 1024  # Example file size limit (2GB)
    ...
    try:
        ...
        ffmpeg_cmd = ['ffmpeg', '-i', downloaded_file, '-map', '0:v']  # Always map video stream
...
async def change_index_subtitle(bot, msg):
    ...
    try:
        ...
        while response is None:
            ...
            if status:
                current_progress = status.progress() * 100
                await progress_message(current_progress, 100, "Uploading to Google Drive", sts, c_time)
        ...
        if CAPTION:
            caption_text = CAPTION.format(file_name=output_filename, file_size=filesize_human)
        else:
            caption_text = f"Uploaded File: {output_filename}\nSize: {filesize_human}"
        ...
        await safe_edit_message(sts, f"Error uploading to Google Drive: {e}")
...
async def start_merge_command(bot, msg: Message):
    ...
    merge_state[msg.from_user.id] = {"files": [], "output_filename": None}
...
async def merge_and_upload(bot, msg: Message):
    ...
    try:
        ...
        for file_msg in files_to_merge:
            file_path = await bot.download_media(file_msg, file_name=DOWNLOAD_LOCATION)
        ...
        # Replace with your merging logic (e.g., ffmpeg)
        # Example: await merge_videos(input_file, output_path)

        # Example caption for the merged file
        ...
        if filesize <= FILE_SIZE_LIMIT:
            ...
            # Upload to Google Drive (example function call)
...
async def link_to_file(bot, msg: Message):
    global RENAME_ENABLED, GDRIVE_FOLDER_ID, DOWNLOAD_LOCATION, CAPTION

    if not RENAME_ENABLED:
        return await msg.reply_text("The leech feature is currently disabled.")
    ...
    try:
        if reply.text and ("seedr" in reply.text or "workers" in reply.text):
            ...
            downloaded = await bot.download_media(message=reply, file_name=new_name, progress=progress_message, progress_args=("Downloading", sts, c_time))

        await upload_file(bot, msg, downloaded, new_name, sts, c_time, media)

    except Exception as e:
        await sts.edit(f"Error during leeching: {e}")
...
async def change_index_audio(bot, msg):
    ...
    if filesize > FILE_SIZE_LIMIT:
        ...
        button = [[InlineKeyboardButton("☁️ CloudUrl ☁️", url=f"{file_link}")]]
        await msg.reply_text(
            f"File successfully Change Index Audio and uploaded to Google Drive!\n\n"
            f"Google Drive Link: [View File]({file_link})\n\n"
            f"Uploaded File: {output_filename}\n"
            f"Request User: {msg.from_user.mention}\n\n"
            f"Size: {filesize_human}",
...
async def change_index_subtitle(bot, msg):
    ...
    if filesize > FILE_SIZE_LIMIT:
        ...
        button = [[InlineKeyboardButton("☁️ CloudUrl ☁️", url=f"{file_link}")]]
        await msg.reply_text(
            f"File successfully Change Index Subtitles and uploaded to Google Drive!\n\n"
            f"Google Drive Link: [View File]({file_link})\n\n"
            f"Uploaded File: {output_filename}\n"
            f"Request User: {msg.from_user.mention}\n\n"
            f"Size: {filesize_human}",
